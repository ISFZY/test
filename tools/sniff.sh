...
watch_traffic() {
    ...
    # === 打印表头 ===
    # 使用 printf 预设固定宽度，确保和下面的 awk 保持一致
    # %-20s 表示左对齐占20格
    printf "${GRAY}%-20s %-27s %-35s %-65s %s${PLAIN}\n" "[时间]" "[来源IP]" "[路由路径]" "[目标地址]" "[用户]"
    echo -e "${GRAY}----------------------------------------------------------------------------------------------------${PLAIN}"
    
    # === 打印数据 ===
    # awk 内部同样使用 printf，宽度必须与上面表头一致
    tail -f "$LOG_FILE" | awk '{
        if ($5 == "accepted") {
            # 颜色定义
            # 36=青(时间), 33=黄(源), 35=紫(路由), 32=绿(目标), 37=白(用户)
            
            # 格式化输出
            # 注意：颜色代码 \033[xxm 不要放在 %-Ns 里面，否则会破坏对齐计算
            # 写法逻辑： [颜色] [占位符] [重置]
            
            printf "\033[36m%-20s\033[0m \033[33m%-27s\033[0m \033[35m%-35s\033[0m \033[32m%-65s\033[0m \033[37m%s\033[0m\n", substr($2,1,12), $4, $7$8$9, $6, $11
        }
    }'
}
